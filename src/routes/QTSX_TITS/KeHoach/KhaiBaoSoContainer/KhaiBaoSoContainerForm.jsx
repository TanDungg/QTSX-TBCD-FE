...
const KhaiBaoSoContainerForm = ({ history, match, permission }) => {
  ...
  const getDonHang = () => {
    const url = `tits_qtsx_SoContainer/get-list-don-hang-chua-dong-cont`;
  ...
  const getListSanPham = (val) => {
    const url = `tits_qtsx_Donhang/${val}`;
    new Promise((resolve, reject) => {
      dispatch(fetchStart(url, "GET", null, "DETAIL", "", resolve, reject));
    })
      .then((res) => {
            ...
            >
              ...
              >
                <Select
                  ...
                  style={{ width: "100%" }}
                  onSelect={(val) => getListSanPham(val)}
                />
              </FormItem>
            </Col>
            <Col
