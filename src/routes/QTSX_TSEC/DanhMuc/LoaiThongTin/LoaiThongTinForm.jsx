...
const LoaiThongTinForm = ({ history, match, permission }) => {
  ...
  const saveData = (formloaithongtin, saveQuit = false) => {
    ...
    if (type === "edit") {
      var newData = { ...formloaithongtin, id: id };
      new Promise((resolve, reject) => {
        dispatch(
          fetchStart(
            `tsec_qtsx_LoaiThongTin/${id}`,
      ...
      >
        ...
        >
          <Col xxl={14} xl={16} lg={18} md={20} sm={24} xs={24}>
            <FormItem
              label="Mã loại thông tin"
              name={["formloaithongtin", "maLoaiThongTin"]}
              rules={[
                {
                  type: "string",
                  required: true,
                ...
                {
                  max: 50,
                  message: "Mã loại thông tin không được quá 50 ký tự",
                },
              ]}
            >
              <Input
                className="input-item"
                placeholder="Nhập mã loại thông tin"
              />
            </FormItem>
          </Col>
          <Col xxl={14} xl={16} lg={18} md={20} sm={24} xs={24}>
