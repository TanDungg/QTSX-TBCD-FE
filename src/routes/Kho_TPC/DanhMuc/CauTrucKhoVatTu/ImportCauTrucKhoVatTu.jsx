...
function ImportCauTrucKhoVatTu({ openModalFS, openModal, loading, refesh }) {
  ...
  const TaiFileMau = () => {
    new Promise((resolve, reject) => {
      dispatch(
        fetchStart(
          `CauTrucKho/ExportFileExcel-vat-tu?donVi_Id=${INFO.donVi_Id}`,
          "POST",
          null,
          "DOWLOAD",
          "",
      ...
      exportExcel("File_Mau_Cau_Truc_Kho_Vat_Tu", res.data.dataexcel);
    });
  };
  ...
  const prop = {
    ...
    okText: "Xác nhận",
    cancelText: "Hủy",
    title: "Xác nhận import cấu trúc kho vật tư",
    onOk: handleSubmit,
  };
  const modalXK = () => {
    Modal(prop);
  ...
  return (
    <AntModal
      title="Import cấu trúc kho vật tư"
      open={openModal}
      width={`80%`}
      closable={true}
...
export default ImportCauTrucKhoVatTu;
